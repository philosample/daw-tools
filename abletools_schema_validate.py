...
def build_targets(catalog_dir: Path) -> list[tuple[Path, Path, str, set[str] | None]]:
    ...
    for suffix in ("", "_user_library", "_preferences"):
        ...
        targets.append((catalog_dir / f"ableton_clip_details{suffix}.jsonl", schema_dir / "ableton_clip_details.schema.json", "jsonl", ignore_scope))
        targets.append((catalog_dir / f"ableton_device_params{suffix}.jsonl", schema_dir / "ableton_device_params.schema.json", "jsonl", ignore_scope))
        targets.append((catalog_dir / f"ableton_routing_details{suffix}.jsonl", schema_dir / "ableton_routing_details.schema.json", "jsonl", ignore_scope))
